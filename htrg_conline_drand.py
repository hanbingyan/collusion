...
for i_episode in range(num_episodes):
    ...
    for t in range(num_sub):
        ...
        next_state_idx = next_state.view(-1).int()
        action_idx = action.view(-1).int()
...
with open('htrg_heat_unique0.pickle', 'wb') as fp:
    pickle.dump(heat_unique0, fp)

with open('htrg_heat_unique1.pickle', 'wb') as fp:
    pickle.dump(heat_unique1, fp)

with open('htrg_heat_freq0.pickle', 'wb') as fp:
    pickle.dump(heat_freq0, fp)

with open('htrg_heat_freq1.pickle', 'wb') as fp:
    pickle.dump(heat_freq1, fp)

# Save last episode all heat values
with open('htrg_lastepisode_heat.pickle', 'wb') as fp:
    pickle.dump(heat_episode.to('cpu').numpy(), fp)

# Save the last network weights
torch.save(policy_net.state_dict(), 'htrg_policy_net.pth')
with open('htrg_classic_Q.pickle', 'wb') as fp:
    pickle.dump(Q.to('cpu').numpy(), fp)


# Save correlations
# with open('htrg_random_corr0.pickle', 'wb') as fp:
#     pickle.dump(random_hist0, fp)

# with open('htrg_random_corr1.pickle', 'wb') as fp:
#     pickle.dump(random_hist1, fp)

# with open('htrg_recent_corr0.pickle', 'wb') as fp:
#     pickle.dump(recent_hist0, fp)

# with open('htrg_recent_corr1.pickle', 'wb') as fp:
...
# fig1 = ax1.get_figure()
# fig1.savefig('htrg_heat1.eps', format='eps', dpi=500, bbox_inches='tight', pad_inches=0.1)
